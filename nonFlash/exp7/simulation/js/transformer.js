			...
			buttonControl.filter(function(btn){
				if(btn.voltage == voltageChoosen){
					...
					if(raise[0].tap<=btn.clickCount){
						...
						popUpText = `The voltage is regulated to: ${leftVoltDown}KV with ${percent}% tap Raised. Number of times tap raised to be clicked: ${btn.clickCount}`;
						showPopup(popUpText);
					}
				}
			...
			buttonControl.filter(function(btn){
				if(btn.voltage == voltageChoosen){
					...
					if(raise[1].tap<=btn.clickCount){
						...
						let percent = 1.25*btn.clickCount;
						popUpText = `The voltage is regulated to: ${rightVoltDown}KV with ${percent}% tap Raised. Number of times tap raised to be clicked: ${btn.clickCount}`;
						showPopup(popUpText);
					}
				}
			...
			buttonControl.filter(function(btn){
				if(btn.voltage == voltageChoosen){
					...
					if(lower[0].tap<=btn.clickCount){
						...
						let percent = 1.25*btn.clickCount;
						popUpText = `The voltage is regulated to: ${leftVoltDown}KV with ${percent}% tap lowered. Number of times tap lowered to be clicked: ${btn.clickCount}`;
						showPopup(popUpText);
					}
				}
			...
			buttonControl.filter(function(btn){
				if(btn.voltage == voltageChoosen){
					...
					if(lower[1].tap<=btn.clickCount){
						...
						let percent = 1.25*btn.clickCount;
						popUpText = `The voltage is regulated to: ${rightVoltDown}KV with ${percent}% tap lowered. Number of times tap lowered to be clicked: ${btn.clickCount}`;
						showPopup(popUpText);
					}
				}
